...
int main() {
    ...
    //Data point copy
    unsigned char * pData = (unsigned char *)outImg.data;
    pData = (unsigned char *)malloc(sizeof(unsigned char)*width*height*3);

    // Setup VDMA handle and memory-mapped ranges
    vdma_setup(&handle, 0x43000000, 1920, 1080, 4, 0x0e000000, 0x0f000000, 0x10000000);
    
    // virtual address of image coming in
    int dh = open("/dev/mem", O_RDWR | O_SYNC); // Open /dev/mem which represents the whole physical memory
    unsigned char* virtual_address = (unsigned char*)mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, 0x0e000000); 
    ...
    memcpy(pData, virtual_address, sizeof(unsigned char)*width*height*3);

    // save file
    cv::imwrite( "/home/linaro/work/vdma/vdma_image.jpg", outImg );
}
